...
static void wifi_acquire_ip(void);
/***************** Declarations ******************/
                          ...
                          void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < NETWORK_WIFI_MAX_RETRY) {
            esp_wifi_connect();
            s_retry_num++;
...
{
    {
        /* Netif subsystem initialize for Wi-Fi subsystem */
    ...
    {
        /* Actual Wi-Fi subsystem init and start associated tasks */
...
static void wifi_acquire_ip(void)
{
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(TAG, "connected to ap SSID:%s password:%s", NETWORK_WIFI_SSID, NETWORK_WIFI_PASS);
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI(TAG, "Failed to connect to SSID:%s, password:%s", NETWORK_WIFI_SSID,
                 NETWORK_WIFI_PASS);
    } else {
